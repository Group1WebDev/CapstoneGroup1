      ...
      <div className='container'>
        <div className='img_prnt'>
          ...
          <div className='info_area'>
            ...
            <div className='icon_content'>
              ...
              <span className='userLocation'>{isUserEditing ? <input className={changeClass} type='text' value={userLocation} onChange={(e) => setUserLocation(e.target.value)} /> : userLocation}</span>
            </div>
          </div>
        ...
        <div className='personal_details'>
          ...
          <div className='profile_input'>
            <label>Bio</label>
            {isUserEditing ? <textarea className={changeClass} cols='30' rows='6' value={userInfo} onChange={(e) => setUserInfo(e.target.value)} /> : <textarea cols='30' rows='6' value={userInfo} disabled></textarea>}
          </div>
          ...
          <div className='oneline_input'>
            ...
            <div className='profile_input'>
              ...
              {isUserEditing ? <input className={changeClass} type='phone' value={userphone} onChange={(e) => setUserPhone(e.target.value)} /> : <input type='phone' value={userphone} disabled />}
            </div>
          </div>
        </div>
            ...
            <ul>
              {userEduDetail.map((edu, index) => (
                <li key={index} className='flexRowContainer'>
                  <h4>{isUserEditing ? <input className={changeClass} type='text' value={edu.edu} onChange={(e) => edu_Update(index, 'edu', e.target.value)} /> : edu.edu}</h4>
                  <span>{isUserEditing ? <input className={changeClass} type='text' value={edu.date} onChange={(e) => edu_Update(index, 'date', e.target.value)} /> : edu.date}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className='profile_input'>
            <ul>
              {userExperience.map((exp, index) => (
                <li key={index} className='flexRowContainer'>
                  <h4>{isUserEditing ? <input className={changeClass} type='text' value={exp.company} onChange={(e) => exp_Update(index, 'company', e.target.value)} /> : exp.company}</h4>
                  <span>{isUserEditing ? <input className={changeClass} type='text' value={exp.position} onChange={(e) => exp_Update(index, 'position', e.target.value)} /> : exp.position}</span>
                  <span>{isUserEditing ? <input className={changeClass} type='text' value={exp.duration} onChange={(e) => exp_Update(index, 'duration', e.target.value)} /> : exp.duration}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className='personal_details'>
          ...
          <div className='oneline_input'>
            <div className='profile_input'>
              ...
              {isUserEditing ? <input className={changeClass} type='text' value={profileDetail.username} onChange={(e) => setProfileDetail({ ...profileDetail, username: e.target.value })} /> : <input type='text' value={profileDetail.username} disabled />}
            </div>

            <div className='profile_input'>
              <label>Password</label>
              {isUserEditing ? <input className={changeClass} type='password' value={profileDetail.password} onChange={(e) => setProfileDetail({ ...profileDetail, password: e.target.value })} /> : <input type='password' value={profileDetail.password} disabled />}
            </div>
          </div>
        </div>
