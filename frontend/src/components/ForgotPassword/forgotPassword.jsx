...
function ForgotPasswordScreen() {
  ...
  const handleOtpRequest = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://group-1-capstone.onrender.com/generateOTP', {
        method: 'POST',
        headers: {
  ...
  const handleSubmitForgotScreen = async (e) => {
    ...
    try {
      const response = await fetch('https://group-1-capstone.onrender.com/forgotPassword', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
